...
public class PurpleHardenedLeatherArmor extends ArmorItem {

    protected final Type slot;
    public static final Component EFFECT = Component.translatable("tooltip.has_purple_effect.tooltip").withStyle(ChatFormatting.GREEN);
    public static final Component NO_EFFECT = Component.translatable("tooltip.has_purple_effect.tooltip").withStyle(ChatFormatting.GRAY);
    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(CustomArmorMaterials.PURPLE_HARDENED_LEATHER,
                            new MobEffectInstance(MobEffects.NIGHT_VISION, 350, 0)).build();

    public PurpleHardenedLeatherArmor(ArmorMaterial material, Type slot, Properties settings) {
    ...
    private void addStatusEffectForMaterial(Player player, ArmorMaterial mapArmorMaterial, MobEffectInstance mapStatusEffect) {
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            ...
            for(MobEffectInstance mobeffectinstance : collection) {
                if(hasCorrectArmorOn(mapArmorMaterial, player) && mobeffectinstance.getEffect() == mapStatusEffect.getEffect() && mobeffectinstance.getDuration() < 250) {
                    player.addEffect(new MobEffectInstance(mapStatusEffect.getEffect(),
                            mapStatusEffect.getDuration(), mapStatusEffect.getAmplifier(), true, false));
                }
            }
