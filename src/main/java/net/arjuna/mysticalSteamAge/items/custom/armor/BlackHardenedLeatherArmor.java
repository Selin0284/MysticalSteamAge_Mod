...
public class BlackHardenedLeatherArmor extends ArmorItem {

    protected final Type slot;
    public static final Component EFFECT = Component.translatable("tooltip.has_black_effect.tooltip").withStyle(ChatFormatting.GREEN);
    public static final Component NO_EFFECT = Component.translatable("tooltip.has_black_effect.tooltip").withStyle(ChatFormatting.GRAY);

    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(CustomArmorMaterials.BLACK_HARDENED_LEATHER,
                            new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 200, 1)).build();

    public BlackHardenedLeatherArmor(ArmorMaterial material, Type slot, Properties settings) {
        ...
        if (player != null) {
            ...
            if (isEquip) {
                if (hasFullSuitOfArmorOn(player)) {
                    if (hasCorrectArmorOn(CustomArmorMaterials.BLACK_HARDENED_LEATHER, player)) {
